...
export default function Record(propss) {
    ...
    const detail = (data) => {
        ...
        navigate('/info', { state: data.id });
    }
    return (
        <div className="hover:cursor-pointer hover:border-2 hover:border-gray-500 border-white-500 p-2  flex w-full flex-row justify-between" onClick={() => detail(data)}>
            <div className="w-3/4 flex flex-row items-center gap-4 flex-start">
                <div className="flex flex-col justify-center">
                    <img src={data.img} alt="AMD Ryzen 7 7800X3D 4.2 GHz 8-Core Processor" className="w-40 h-32" />
                    <div className="flex flex-row gap-2 items-center justify-center">
                        <img alt="Rating" className="w-4 h-12 " src="https://pc-builder.io/img/star.svg" /> <span>{data.review}</span>
                    </div>
                </div>
                <div className="h-full py-4 flex w-full flex-col justify-between">
                    <a href="https://pc-builder.io/cpu/amd-ryzen-7-7800x3d-p52516" className=" font-black text-bold text-2xl ">{data.title}</a>
                    <div className="flex flex-row w-full justify-between">
                        <div className="w-1/2 gap-2 flex flex-col justify-start">
                            <div className="col-lg-6 col-md-6 col-12 gy-1 col-12">
                                <span>Core Count:</span><span>{data.cores}</span>
                            </div>
                            <div className="col-lg-6 col-md-6 col-12 gy-1 col-12">
                                <span>Boost Clock:</span><span>{data.clock}</span>
                            </div>
                            <div className="col-lg-6 col-md-6 col-12 gy-1 col-12">
                                <span>Integrated Graphics:</span><span>{data.graphics}</span>
                            </div>
                        </div>
                        <div className="w-1/2 gap-2 flex flex-col justify-start">
                            <div className="col-lg-6 col-md-6 col-12 gy-1 col-12">
                                <span>Core Clock:</span><span>{data.cclock}</span>
                            </div>
                            <div className="col-lg-6 col-md-6 col-12 gy-1 col-12">
                                <span>TDP:</span><span>{data.TDP}W</span>
                            </div>
                            <div className="col-lg-6 col-md-6 col-12 gy-1 col-12">
                                <span>Simultaneous Multithreading:</span><span>{data.multithread}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex flex-col justify-between py-4 ">
                <div className="flex justify-end w-full font-black text-bold text-xl ">${data.price}</div>
                <div className="flex flex-row w-full items-center  gap-4 ">
                    ...
                    <div className="py-4 px-12 rounded-md bg-gradient-to-r from-green-400 to-blue-500">+ Add to Build</div>
                </div>
            </div>
        </div>
    )
